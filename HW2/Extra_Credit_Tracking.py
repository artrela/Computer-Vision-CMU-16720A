...
def updatefig(i):
    ...
    im.set_array(seq[:, :, i])
    if i in plot_idx:
        plt.savefig("result/tracking_1_4_walking_" + str(i) + ".png")
    return im, patch,
...
seq = np.load("../data/walking.npy")
rect_0 = np.array([1092, 133, 1208, 381]).T
rect = np.copy(rect_0)
plot_idx = [0, 50, 80, 120, 150]
threshold_drift = 8
...
# np.save("walking-wcrt.npy", lk_res)

####### Visualization #######

lk_res = np.load("walking-wcrt.npy")
...
for i in range(5):
    axarr[i].imshow(plt.imread(
        f"result/tracking_1_4_walking_" + str(plot_idx[i]) + ".png"))
    axarr[i].axis('off')
    axarr[i].axis('tight')
    axarr[i].axis('image')
plt.show()
